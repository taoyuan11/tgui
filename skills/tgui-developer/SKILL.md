---
name: tgui-developer
description: Build, modify, review, and document the local tgui Rust GUI framework. Use when working in the tgui repository on MVVM APIs, widgets, layout, rendering, theme tokens, animation, text input, notifications, media loading, dialogs, native window controls, custom window chrome, optional video support, examples, Cargo features, tests, or project-specific maintenance.
---
...
For text selection, caret, IME, `Input`, or `Textarea` work, read `src/ui/widget/input.rs`, `src/ui/widget/textarea.rs`, `src/ui/widget/common.rs`, `src/ui/widget/core/`, `src/runtime/input.rs`, and `src/runtime/mod.rs`.
...
1. Identify the surface area: public API, example app, widget builder, runtime behavior, text input, renderer/shader, theme token, notifications, media/dialog/window-control/video, platform, or docs.
