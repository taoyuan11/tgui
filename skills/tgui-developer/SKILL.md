---
name: tgui-developer
description: Build, modify, review, and document the local tgui Rust GUI framework. Use when working in the tgui repository on MVVM APIs, widgets, layout, rendering, theme tokens, animation, media loading, dialogs, optional video support, examples, Cargo features, tests, or project-specific maintenance.
---
...
1. Identify the surface area: public API, example app, widget builder, runtime behavior, renderer/shader, theme token, media/dialog/video, platform, or docs.
...
Prefer module tests for `runtime.rs`, `src/ui/widget/core.rs`, `src/media/mod.rs`, `src/animation.rs`, and video backend changes. Running an example is useful for smoke testing, but it is not a substitute for focused tests when shared behavior changes.
