---
name: tgui-developer
description: Build, modify, review, and document the local tgui Rust GUI framework. Use when working in the tgui repository on MVVM APIs, widgets, layout, rendering, theme tokens, animation, notifications, media loading, dialogs, native window controls, custom window chrome, optional video support, examples, Cargo features, tests, or project-specific maintenance.
---
...
For text selection, caret, or IME work, read `src/ui/widget/common.rs`, `src/ui/widget/core.rs`, and `src/runtime.rs`.
...
- There is no public `Input` or `TextArea` widget in the current tree. Treat text-editing code as shared selection, caret, IME, and copy infrastructure unless a new public API is being introduced intentionally.
