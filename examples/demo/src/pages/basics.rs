...
const CODE_SHADOW_LAYOUT: &str = r#"Flex::horizontal().gap(dp(18.0)).child(el![
    Stack::new().size(dp(72.0), dp(72.0)).style(shadow_showcase_style),
    Stack::new().size(dp(100.0), dp(100.0)).style(shadow_showcase_style),
])"#;
        ...
        vec![
            UsageDemo::new(
                ...
                Text::new("这是一段可直接渲染的文本组件。")
                    .style(|mode| styles::text_style(mode, sp(16.0))),
                CODE_TEXT_BASIC,
            ),
            UsageDemo::new(
                "text/selectable",
    ...
    demo_section::component_doc(
        ...
        vec![
            UsageDemo::new(
                ...
                "水平分隔线保持内容层次清晰。",
                Flex::vertical().gap(dp(8.0)).child(el![
                    Text::new("上方内容").style(styles::status_style),
                    Divider::new().width(dp(280.0)),
                    Text::new("下方内容").style(styles::status_style),
                ]),
                CODE_DIVIDER_BASIC,
            ),
            UsageDemo::new(
                ...
                Flex::vertical().gap(dp(10.0)).child(el![
                    ...
                    Flex::horizontal().gap(dp(12.0)).child(el![
                        Text::new("左侧").style(styles::status_style),
                        Divider::new().vertical().height(dp(24.0)),
                        Text::new("右侧").style(styles::status_style),
                    ]),
                ]),
        ...
        vec![
            UsageDemo::new(
                ...
                Stack::new().size(dp(180.0), dp(150.0)).center().child(
                    Stack::new()
                        .size(dp(100.0), dp(100.0))
                        .style(styles::shadow_showcase_style),
                ),
                CODE_SHADOW_SURFACE,
            ),
            UsageDemo::new(
