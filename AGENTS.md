...
`tgui` 是一个 Rust GUI 框架 crate，目标是提供基于 `wgpu` 的 GPU 加速渲染、MVVM 状态模型、`taffy` 布局、声明式组件树、主题系统、动画、媒体加载、对话框以及可选视频播放能力。
...
- `src/application/mod.rs`：`Application`、`ApplicationBuilder`、`WindowSpec`、多窗口声明与运行入口。
- `src/runtime.rs`：运行时核心，管理事件循环、窗口生命周期、输入、焦点、滚动、文本编辑、命令派发、异步对话框回调、主题绑定、动画刷新、媒体状态和渲染调度。
- `src/foundation/`：基础能力，包括 `Observable`、`Binding`、`ViewModelContext`、`Command`、`ValueCommand`、`InputTrigger`、`TguiError`、`Color`。
...
- `src/rendering/renderer.rs`：`wgpu` 渲染器，包含矩形、渐变/brush、mesh、文字、纹理、backdrop blur 等 pipeline。
...
7. `src/ui/widget/core.rs` 中的 `Element`、`WidgetTree`、布局和渲染输出相关部分
8. `src/runtime.rs` 的 `BoundRuntime`、`BoundRuntimeHandler` 和事件处理部分
9. 需要改渲染时再读 `src/rendering/renderer.rs` 与 `src/rendering/shader/*`
