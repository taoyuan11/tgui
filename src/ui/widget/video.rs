...
use crate::media::{ContentFit, MediaSource, VideoConfig, VideoControllerHandle};
...
impl Video {
    ...
    pub(crate) fn config(&self) -> VideoConfig {
        VideoConfig {
    ...
    pub fn controller(mut self, controller: VideoControllerHandle) -> Self {
    ...
    pub fn on_play<VM>(self, command: Command<VM>) -> Element<VM> {
        self.into_element_with_media_events(MediaEventHandlers {
            on_play: Some(command),
            ..Default::default()
        })
    }

    pub fn on_pause<VM>(self, command: Command<VM>) -> Element<VM> {
        self.into_element_with_media_events(MediaEventHandlers {
            on_pause: Some(command),
            ..Default::default()
        })
    }

    pub fn on_resume<VM>(self, command: Command<VM>) -> Element<VM> {
        self.into_element_with_media_events(MediaEventHandlers {
            on_resume: Some(command),
            ..Default::default()
        })
    }

    pub fn on_end<VM>(self, command: Command<VM>) -> Element<VM> {
        self.into_element_with_media_events(MediaEventHandlers {
            on_end: Some(command),
            ..Default::default()
        })
    ...
    pub fn on_seek<VM>(self, command: Command<VM>) -> Element<VM> {
        self.into_element_with_media_events(MediaEventHandlers {
            on_seek: Some(command),
            ..Default::default()
        })
    }
    ...
    fn into_element_with_interactions<VM>(
        ...
        Element {
            ...
            kind: WidgetKind::Video { video: self },
        }
    }
...
impl<VM> From<Video> for Element<VM> {
    fn from(value: Video) -> Self {
        Element {
            ...
            kind: WidgetKind::Video { video: value },
        }
    }
}
