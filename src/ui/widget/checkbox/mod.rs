...
impl<VM> Checkbox<VM> {
    ...
    pub fn style(
        mut self,
        mutator: impl Fn(&mut CheckboxStyle, &StyleContext<'_>) + Send + Sync + 'static,
    ) -> Self {
        if let WidgetKind::Checkbox { style, .. } = &mut self.element.kind {
            *style = Some(super::style::StyleResolver::mutate(
                |context| CheckboxStyle::default_for_theme(context.theme),
                mutator,
            ));
        }
        self
    ...
    pub fn style_full(
        mut self,
        resolver: impl Fn(&StyleContext<'_>) -> CheckboxStyle + Send + Sync + 'static,
    ) -> Self {
        if let WidgetKind::Checkbox { style, .. } = &mut self.element.kind {
    ...
    pub fn on_change(mut self, command: ValueCommand<VM, bool>) -> Self {
        if let WidgetKind::Checkbox { on_change, .. } = &mut self.element.kind {
...
impl<VM> From<Checkbox<VM>> for Element<VM> {
    fn from(value: Checkbox<VM>) -> Self {
        value.element
    }
}
