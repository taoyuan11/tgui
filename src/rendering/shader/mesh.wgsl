struct VertexInput {
    ...
    @location(2) brush_meta: vec4<f32>,
    @location(3) gradient_data0: vec4<f32>,
    @location(4) gradient_data1: vec4<f32>,
    @location(5) stop_offsets0: vec4<f32>,
    @location(6) stop_offsets1: vec4<f32>,
    ...
    @location(11) stop_color4: vec4<f32>,
    @location(12) stop_color5: vec4<f32>,
    @location(13) stop_color6: vec4<f32>,
...
struct VertexOutput {
    ...
    @location(1) brush_meta: vec4<f32>,
    @location(2) gradient_data0: vec4<f32>,
    @location(3) gradient_data1: vec4<f32>,
    @location(4) stop_offsets0: vec4<f32>,
    @location(5) stop_offsets1: vec4<f32>,
    @location(6) stop_color0: vec4<f32>,
    @location(7) stop_color1: vec4<f32>,
    @location(8) stop_color2: vec4<f32>,
    @location(9) stop_color3: vec4<f32>,
    @location(10) stop_color4: vec4<f32>,
    @location(11) stop_color5: vec4<f32>,
...
fn stop_offset(input: VertexOutput, index: i32) -> f32 {
    switch index {
        ...
        default: { return input.stop_offsets1.w; }
    }
}
...
fn stop_color(input: VertexOutput, index: i32) -> vec4<f32> {
    switch index {
        ...
        case 6: { return input.stop_color6; }
        default: { return input.stop_color7; }
    }
}
...
fn gradient_color(input: VertexOutput, t_raw: f32) -> vec4<f32> {
    ...
    for (var index = 1; index < 8; index = index + 1) {
        if index >= count {
            break;
        }
...
fn vs_main(input: VertexInput) -> VertexOutput {
    ...
    output.stop_color7 = input.stop_color7;
    return output;
}
