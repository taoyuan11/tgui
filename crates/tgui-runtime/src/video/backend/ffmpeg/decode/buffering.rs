...
impl DecodeSession {
    ...
    pub(super) fn fill_ready_video_frames(
            ...
            && (!respect_buffer_memory_limit || !self.buffering_constrained_by_memory_limit())
        {
            ...
            let mut newly_decoded = Vec::with_capacity(decode_budget);
                ...
                && (!respect_buffer_memory_limit || !self.buffering_constrained_by_memory_limit())
            {
                ...
                let position =
                    pts_to_duration(self.decoded_video_frame.timestamp(), self.video_time_base)
                        .unwrap_or_else(|| {
                            self.queued_video_tail_position()
                                .unwrap_or(self.start_position)
                        });
                ...
                let frame_texture = self.video_converter.convert_render_frame(
                    &self.decoded_video_frame,
                    self.target_raster,
                    self.video_texture_id,
                    revision,
                )?;
                let decoded_bytes = frame_texture.decoded_bytes();
        ...
        if decode_budget > 0
            ...
            && (!respect_buffer_memory_limit || !self.buffering_constrained_by_memory_limit())
        {
            let mut flushed_frames = Vec::with_capacity(decode_budget);
            while decode_budget > 0
                && (!respect_buffer_memory_limit || !self.buffering_constrained_by_memory_limit())
            {
                match self
